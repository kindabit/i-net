...
| # | 操作 | 预期结果 |
|---|------|----------|
| 1 | 点击右下角「工具」菜单按钮，断言菜单项「KeePass 2.0数据导入」「导出数据库」 | 菜单展开；菜单面板与右下角按钮条之间保持约 8px 间距、不重叠（菜单底边距「工具」按钮顶边约 12px；150% 缩放下 UI 树物理像素约 18px）；截图留档 |
...
11. **步骤 1 追加间距断言**：工具菜单面板与右下角按钮条不重叠（菜单底边距「工具」按钮顶边
    约 12px，物理像素 18px@150% 缩放）；同时解锁步骤适配首页「自动填入最后打开的数据库名称」
    功能（名称输入改为全选覆盖后输入）。

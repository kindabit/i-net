...
/// 数据迁移聚合导入：成功路径（画布与画布节点创建、节点、字段与父子边写入、全部操作聚合为
/// 恰好一条日志）、画布重名去重（重名时追加 " 2"）、失败路径（command 层空画布名、
/// 字段名重复、悬空字典引用与非法边下标均不写库），以及迁移源文件字节读取接口的成功与失败路径。
#[test]
fn test_user_database_migration_import_keepass2() {
    let _guard = test::acquire_test_lock();
    ...
    assert!(matches!(
        migration::command::import_keepass2::preprocess(
            String::new(),
            0.0,
            240.0,
            nodes.clone(),
    ...
    assert!(matches!(
        migration::service::import_keepass2("test", 0.0, 240.0, &duplicated, &edges),
    ...
    assert!(matches!(
        migration::service::import_keepass2("test", 0.0, 240.0, &dangling, &edges),
        Err(ErrorCode::NoDictionaryEntryWithSuchId { .. })
    ));
    assert_eq!(canvas::service::list(false).unwrap().len(), 1);
    assert_eq!(log::service::list(0, 1, LogFilter::default()).unwrap().total, 0);
    ...
    assert!(matches!(
        migration::service::import_keepass2("test", 0.0, 240.0, &nodes, &backward_edge),
    ...
    assert!(matches!(
        migration::service::import_keepass2("test", 0.0, 240.0, &nodes, &out_of_range_edge),
    ...
    let imported_id =
        migration::service::import_keepass2("test", 0.0, 240.0, &nodes, &edges).unwrap();
    ...
    assert_eq!(
        migration::command::read_file::read_file_bytes(file_path.to_str().unwrap()).unwrap(),
        content
    );
    // command 层 preprocess 成功路径：有效 .kdbx 路径返回 Ok。
    assert!(
        migration::command::read_file::preprocess(file_path.to_string_lossy().to_string()).is_ok()
    );
    ...
    assert!(
        migration::command::read_file::preprocess(upper_path.to_string_lossy().to_string()).is_ok()
    );
    ...
    assert!(matches!(
        migration::command::read_file::preprocess(wrong_ext_path.to_string_lossy().to_string()),
    ...
    assert!(matches!(
        migration::command::read_file::preprocess(no_ext_path.to_string_lossy().to_string()),
        Err(ErrorCode::InvalidKdbxFileExtension { .. })
    ));

    // 失败路径：空路径（空串与纯空白）返回 EmptyFilePath。
    assert!(matches!(
        migration::command::read_file::preprocess(String::new()),
        Err(ErrorCode::EmptyFilePath)
    ));
    assert!(matches!(
        migration::command::read_file::preprocess("  ".to_string()),
        Err(ErrorCode::EmptyFilePath)
    ));
    ...
    assert!(matches!(
        migration::command::read_file::preprocess(missing_path.to_string_lossy().to_string()),
        Err(ErrorCode::FailToReadFile { .. })
    ));
    assert!(matches!(
        migration::command::read_file::read_file_bytes(missing_path.to_str().unwrap()),
        Err(ErrorCode::FailToReadFile { .. })
    ));

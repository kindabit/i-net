...
/// 新连接规则矩阵：
/// - 数据节点 → 数据节点：仅插边，无影子。
/// - 数据节点 / 入向影子 → 画布数据节点：在 target 引用的子画布内创建源端本体的入向影子。
/// - 数据节点 → 出向影子：沿目标影子的产生边链解析到本体画布数据节点，在其引用的子画布内创建源端本体的入向影子。
/// - 画布数据节点 → 画布数据节点 / 出向影子：在 source 引用的子画布内创建目标端本体的出向影子。
/// - 入向影子 → 数据节点：仅插边，无影子。
///
/// 非法连接：
/// - 画布数据节点 → 数据节点：`ErrorCode::CanvasToDataNodeEdge`（避免依赖项散落各画布）。
/// - 入向影子作为目标 / 出向影子作为源：`ErrorCode::InvalidShadowEdge`。
/// - 入向影子 → 出向影子：`ErrorCode::ShadowToShadowEdge`（应在父画布直接连线）。
///
/// 影子生命周期由边控制：影子由产生它的边标识，删除边时其产生的影子
/// 经 node.shadow_producing_edge_id 外键级联删除，影子的相连边经 edge.source_id/target_id 外键级联删除，
...
/// - `confirmed`: 调用方已确认反向替换路径中影子删除带来的断连影响；同向重建与无旧边流程忽略。
///
/// # 返回值
/// 返回新建或更新后的边；任一节点不存在时返回 `ErrorCode::NoNodeWithSuchId`，
/// 两端连接桩相同时返回 `ErrorCode::EdgeSameHandle`，
/// 新建该边会成环时返回 `ErrorCode::EdgeWouldFormCycle`，
/// 画布数据节点作为源连接数据节点时返回 `ErrorCode::CanvasToDataNodeEdge`，
...
pub fn create(
    ...
    if source_handle == target_handle {
        return Err(ErrorCode::EdgeSameHandle);
    }
    ...
    if let Some(mut old) = dao::select_between(&connection, source_id, target_id)? {
        dao::update_handles(&connection, &old.id, &source_handle, &target_handle)?;
        old.source_handle = source_handle;
        old.target_handle = target_handle;
        return Ok(old);
    }
    ...
    if let Some(old) = &old_edge {
        ...
        // 删除旧边：其产生的影子经 node.shadow_producing_edge_id 外键级联删除，下游嵌套影子沿
        // edge.source_id/target_id 与 node.shadow_producing_edge_id 外键链递归级联，应用层禁止手写递归删除。
...
///
/// # 返回值
/// 连线合法时返回 `Ok(())`；画布数据节点连接数据节点时返回 `ErrorCode::CanvasToDataNodeEdge`，
/// 影子节点连线方向不合法时返回 `ErrorCode::InvalidShadowEdge`，
...
fn validate_shadow_endpoints(
    ...
    // 画布数据节点不能直接作为源连接数据节点（避免目标节点的依赖项散落在各画布中，
    // 应先经子画布中转）。
    if source.canvas_ref_id.is_some() && target.canvas_ref_id.is_none() && target.shadow_producing_edge_id.is_none()
    {
        return Err(ErrorCode::CanvasToDataNodeEdge);
    }
    // 入向影子（数据节点的影子）只能作为源，不能作为目标。
    ...
    if source.shadow_producing_edge_id.is_some() && target.shadow_producing_edge_id.is_some() {
...
mod tests {
    ...
    /// 影子方向翻转、影子端点校验与同 handle 校验在替换路径下仍然先于边存在性判断生效。
    #[test]
    fn test_edge_replace() {
        ...
        // 新机制下画布内对某本体节点的影子是当前生效产生边的影子，需传入最新边 id 才能查到当前影子。
        ...
        // 同向用不同连接桩重建 → 触发连接桩更新路径。
        let log_total_before_handle = log::service::list(0, 1, LogFilter::default()).unwrap().total;
        let edge_ab_updated = edge::service::create(&root.id, &node_a.id, "top".to_string(), &node_b.id, "bottom".to_string(), false).unwrap();
        // 边 id 不变，连接桩为新值，title/description 保留。
        assert_eq!(edge_ab_updated.id, edge_ab.id);
        assert_eq!(edge_ab_updated.source_handle, "top");
        assert_eq!(edge_ab_updated.target_handle, "bottom");
        ...
        // 日志总数不变（同向重建不记任何日志）。
        assert_eq!(log::service::list(0, 1, LogFilter::default()).unwrap().total, log_total_before_handle);
        ...
        assert_eq!(edge_ab_idempotent.source_handle, "top");
        assert_eq!(edge_ab_idempotent.target_handle, "bottom");
        assert_eq!(log::service::list(0, 1, LogFilter::default()).unwrap().total, log_total_before_handle);
        ...
        // ===== 第 4 阶段：同向 handle 更新不影响影子及其连接 =====
        // 准备：根画布内画布数据节点 A、B（分别引用画布 a、画布 b），建边 A→B 在 a 内产生 B 的出向影子。
        // 新规则下画布数据节点→数据节点被禁止，故两端均用画布数据节点以保留影子+连线场景。
        ...
        // 画布 a 内建数据节点 M1 并建边 shadow_b→M1（出向影子不能作 source，但入向可以；
        // 此处改用画布 a 内数据节点 M1，shadow_b 是出向影子，故边方向 M1 → shadow_b）。
        ...
        // 同向重建 A→B（handle 变换）→ 直接成功（同向已有边走连接桩更新路径），
        // 影子仍由同一条边产生，shadow_producing_edge_id 与 id 都不变，画布 a 内 M1→shadow_b 边保留。
        ...
        assert_eq!(edge_ab_updated.source_handle, "top");
        assert_eq!(edge_ab_updated.target_handle, "bottom");
        // 影子仍由同一产生边产生：id 与 shadow_producing_edge_id 都保持不变，画布 a 内 M1→shadow_b 边保留。
        let shadow_b_unchanged = shadow_by_edge(&edge_ab.id).unwrap();
        assert_eq!(shadow_b_unchanged.id, shadow_b.id);
        assert_eq!(shadow_b_unchanged.shadow_producing_edge_id.as_deref(), Some(edge_ab.id.as_str()));
        assert!(edge::service::list(&canvas_a).unwrap().iter().any(|e| e.id == edge_m_s.id));
        // 影子方向仍为 Outflow（产生边源端是画布数据节点 A）。
        ...
        // confirmed=true 重调 → 成功，根画布边为 B→A，旧边 A→B 被删除，旧影子经 shadow_producing_edge_id 外键级联消失；
        // 新边 B→A 在 source.canvas_ref_id 画布 b 内产生新出向影子（目标本体 = A）。
        ...
        // 画布 b 内建数据节点 M2；尝试在 b 内建 shadow_a_in_b（出向影子）→ M2（出向影子作 source）应被拦截。
        ...
        // ===== 第 7 阶段：替换路径同 handle 检查仍先生效 =====
        // 准备：在第 5 步状态下 B→A 已存在；尝试用相同 handle 重建 B→A 应被 EdgeSameHandle 拦截。
        assert!(matches!(

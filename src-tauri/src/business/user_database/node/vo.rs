...
/// 影子节点的 title / subtitle / color 沿产生边链合并自本体节点；canvas_ref_id 恒为 None
/// （影子节点本身不能是画布数据节点；出向影子的本体是画布数据节点，其 canvas_ref_id 不合并给影子，
/// 改由 shadow_origin_canvas_ref_id 单独携带）；数据节点的四个扩展字段均为 None。
#[derive(Debug, Clone, Serialize)]
pub struct NodeVO {
    /// 节点基础字段（serde flatten 展开）。
    #[serde(flatten)]
    pub node: Node,
    /// 影子节点本体节点的 id，仅影子节点有值；供前端跳转定位。
    ...
    /// 影子节点本体（画布数据节点）引用的子画布 id，仅出向影子有值；供前端双击影子节点时跳转定位。
    /// 数据节点与入向影子为 None。
...
impl std::ops::Deref for NodeVO {
    type Target = Node;
    /// 解引用到节点基础字段，使调用方可以直接访问 Node 的字段（如 n.id、n.title）。
    fn deref(&self) -> &Node {
        &self.node
    }
}

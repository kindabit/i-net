...
#[tauri::command]
pub fn user_database_data_node_set_color(id: String, color: String) -> Result<(), ErrorCode> {
    preprocess(id, color)
}

/// `user_database_data_node_set_color` 的 preprocess 函数：校验参数后接入 service 层的 set_color 函数。
///
/// 颜色仅裁剪首尾空白字符，不做格式校验。

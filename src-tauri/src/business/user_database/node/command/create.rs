...
///
/// 有两种模式：
/// - `create_canvas == false`（数据节点）：可选地基于模板 id 复制模板字段结构。
/// - `create_canvas == true`（画布数据节点）：以 title 作为画布名称创建子画布，并在宿主画布内创建引用节点；
...
pub fn user_database_node_create(
    ...
    preprocess(canvas_id, title, subtitle, x, y, template_id, create_canvas)
}

...
/// 初始数据；如果已存在，则用密钥解密打开。打开成功后先更新元数据的最后打开时间
/// 并写回元数据数据库，再将连接、更新后的元数据和密钥写入 user_database 的 state。
///
/// # 参数
/// - `id`: 数据库 id。
/// - `key`: 32 字节的密钥（密码哈希得到）。
///
/// # 返回值
/// 返回更新过最后打开时间的元数据；id 不存在时返回 `ErrorCode::NoUserDatabaseWithSuchId`，
...
pub fn initialize(id: &str, key: [u8; 32]) -> Result<Metadata, ErrorCode> {
    ...
    // 打开成功后先更新最后打开时间并写回元数据数据库，再把更新后的元数据写入 state，
    // 保证 state 中保存的和返回值都是更新后的元数据。

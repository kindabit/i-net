...
use crate::business::user_database::shadow::service::resolve_origin;
use crate::error_code::ErrorCode;

/// 取节点用于日志载荷的展示标题：影子节点的标题落库为空串（影子自有数据仅位置与产生边引用，
/// 展示数据从本体节点拉取），故影子节点沿产生边链解析本体并取本体标题；
/// 非影子节点的本体是其自身，直接取自身标题。
///
/// # 参数
/// - `connection`: 数据库连接。
...
pub fn display_title(connection: &Connection, node: &Node) -> Result<String, ErrorCode> {
    ...
    Ok(resolve_origin(connection, node)?.title)
}

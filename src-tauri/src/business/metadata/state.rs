...
/// 全局静态变量，存储内含 metadata 数据库的 sqlite connection。
///
...
/// 将 metadata 数据库连接写入全局状态。
///
/// # 参数
/// - `connection`: metadata 数据库连接。
///
/// # 返回值
/// 无。
...
/// metadata 数据库连接的锁守卫，解引用后直接得到 `&Connection`。
///
...
impl Deref for ConnectionGuard {
    ...
    fn deref(&self) -> &Connection {
        self.connection
            .as_ref()
            .expect("metadata database connection is not initialized")
    }
}

/// 获取 metadata 数据库连接的锁守卫。
///

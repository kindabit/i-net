 ...
 * 全部对外能力（t / te / tm / d 不依赖组件上下文，setup、模板、普通模块中均可
 * 直接使用；模板中调用会自动追踪当前语言，随语言切换重渲染）：
 * - mountI18n：加载持久化语言偏好并挂载 i18n 至 Vue 应用（异步，应用启动时调用一次）
 * - t / te / tm / d：文案翻译 / 文案键存在性检查 / 翻译消息（未插值的原始定义，用于枚举嵌套结构的键）/ 日期时间格式化
...
/** 翻译消息（任意上下文可用；返回未插值的原始消息定义，用于枚举嵌套结构的键） */
export const tm = composer.tm;

...
function node(id: string): AutoLayoutNode {
  return { id, width: DATA_NODE_WIDTH, height: DATA_NODE_HEIGHT };
}

/** 构造测试用边。 */
function edge(source: string, target: string): AutoLayoutEdge {
  return { source, target };
}

/** 构造带连接桩方向的测试用边。 */
function edgeWithHandles(
  source: string,
  target: string,
  sourceHandle?: Handle,
  targetHandle?: Handle,
...
/** 计算两个节点中心坐标之间的欧氏距离。 */
function distance(a: AutoLayoutPoint, b: AutoLayoutPoint): number {
...
describe("computeAutoLayout", () => {
  it("空图返回空结果", () => {
    ...
    expect(computeAutoLayout([], [], CONFIG).size).toBe(0);
  });

  it("单节点位于原点", () => {
    // 意图：唯一节点没有边，属于孤立节点，归一化后应恰好位于包围盒中心（原点）。
    const result = computeAutoLayout([node("a")], [], CONFIG);
  ...
  it("链式图：各层等距、共线、同向", () => {
    ...
    // 验证无连接桩边沿父节点流向方向锚定（B 在 A 正东、C 继续向东）。
  ...
  it("星型图：无连接桩子代绕根节点均布成正方形", () => {
    // 意图：根节点 4 个无连接桩子代绕根均布（相邻角差 90°，即正方形），
    // 验证孤立根的无连接桩子代均布方向分配与恒距。
  ...
  it("确定性：同输入多次调用、输入乱序，输出完全一致", () => {
    ...
    const first = computeAutoLayout(buildInput().nodes, buildInput().edges, CONFIG);
    const secondInput = buildInput();
    const second = computeAutoLayout(secondInput.nodes, secondInput.edges, CONFIG);
    const reversed = computeAutoLayout(
  ...
  it("均布容量：无连接桩子代过密时自动扩大锚距", () => {
    // 意图：root + 30 个无连接桩子代，理想锚距按弦长公式 max(ringSpacing, span/(2·sin(π/n))) 计算，
  ...
  it("均布平局按 id 排序", () => {
    // 意图：root + id-a、id-b 两个无连接桩子代，锚距 = max(300, span/(2·sin(π/2))) = 300；
  ...
  it("多连接桩父节点：理想位为各入边锚点的均值", () => {
    // 意图：a1、a2、a3 为根节点，水平成行居中于原点（a1(−200,0)、a2(0,0)、a3(200,0)）；
    // edgeWithHandles(a1,b,"bottom") 与 edgeWithHandles(a2,b,"bottom") 的锚点分别为
    ...
    const result = computeAutoLayout(
      ...
      [edgeWithHandles("a1", "b", "bottom"), edgeWithHandles("a2", "b", "bottom")],
      CONFIG,
    );
  ...
  it("圆心父 + 双连接桩方位：连接桩期望方向决定子代位置", () => {
    // 意图：唯一根 a 在圆心，边 a→b 带 sourceHandle=right(0)、targetHandle=left(π+π≡0)，
    // 两连接桩期望方向均为正东，子代 b 应落在 a 的正东方向，间距为 ringSpacing。
    const result = computeAutoLayout(
      [node("a"), node("b")],
      [edgeWithHandles("a", "b", "right", "left")],
      CONFIG,
    );
    const a = result.get("a")!;
    const b = result.get("b")!;
  ...
  it("仅 sourceHandle：子代沿连接桩期望方向展开", () => {
    // 意图：唯一根 a 在圆心，边 a→b 仅带 sourceHandle=bottom(π/2)，
    // 子代 b 应落在 a 的正南方向，间距为 ringSpacing。
    const result = computeAutoLayout(
      [node("a"), node("b")],
      [edgeWithHandles("a", "b", "bottom")],
      CONFIG,
    );
    const a = result.get("a")!;
    const b = result.get("b")!;
  ...
  it("仅 targetHandle（反向语义）：进入方向取反后决定子代方位", () => {
    // 意图：唯一根 a 在圆心，边 a→b 仅带 targetHandle=top，top 的反向 = bottom(π/2)，
    // 子代 b 应落在 a 正南，与仅 sourceHandle=bottom 的行为一致。
    const result = computeAutoLayout(
      [node("a"), node("b")],
      [edgeWithHandles("a", "b", undefined, "top")],
      CONFIG,
    );
    const a = result.get("a")!;
    const b = result.get("b")!;
  ...
  it("非根父节点的连接桩方向精确锚定", () => {
    // 意图：r 为根节点，edge(r,m) 无连接桩（m 在正东 300）；
    // edgeWithHandles(m,c,"bottom") 的锚点 = m + (0,300)，新语义为连接桩方向精确锚定
    //（非角度偏置），c 应落在 m 正南 300 处。断言：c.cx ≈ m.cx（6 位）；
    // c.cy − m.cy ≈ 300（6 位）。
    const result = computeAutoLayout(
      [node("r"), node("m"), node("c")],
      [edge("r", "m"), edgeWithHandles("m", "c", "bottom")],
      CONFIG,
    );
  ...
  it("同连接桩兄弟成列且组间分侧", () => {
    // 意图：r 根，edge(r,pa)、edge(r,pb) 无连接桩（pa 正东 (300,0)、pb 正西 (−300,0)）；
    // pa→a1,a2,a3 均 edgeWithHandles(pa,…,"right")，理想位 = pa+(300,0)，
    // 同父同锚向单入边兄弟沿锚向垂直方向（竖直）等距排开：cy 偏移 −200, 0, +200；
    // pb→b1,b2,b3 均 edgeWithHandles(pb,…,"left")，理想位 = pb−(300,0)。
    ...
    const result = computeAutoLayout(
      ...
      [
        edge("r", "pa"),
        edge("r", "pb"),
        ...aGroup.map((id) => edgeWithHandles("pa", id, "right")),
        ...bGroup.map((id) => edgeWithHandles("pb", id, "left")),
      ],
      CONFIG,
    );
  ...
  it("宇宙风格右向逐列树", () => {
    // 意图：root→c1,c2,c3 均 edgeWithHandles(root,…,"right","left")，
    // 理想位 = root+(300,0)，同父同锚向单入边兄弟沿锚向垂直方向（竖直）等距排开：
    // c1(300,−200)、c2(300,0)、c3(300,+200)；c1→g1 同样连接桩，g1 在 c1 正东 300。
    ...
    const result = computeAutoLayout(
      ...
      [
        ...children.map((id) => edgeWithHandles("root", id, "right", "left")),
        edgeWithHandles("c1", "g1", "right", "left"),
      ],
      CONFIG,
    );
    const root = result.get("root")!;
  ...
  it("同连接桩 5 子成列且中心对称", () => {
    // 意图：root→k1..k5 均 edgeWithHandles(root,…,"right")，理想位 = root+(300,0)，
    ...
    const result = computeAutoLayout(
      ...
      kids.map((id) => edgeWithHandles("root", id, "right")),
      CONFIG,
    );
    const root = result.get("root")!;
  ...
  it("无连接桩 8 子成正八边形", () => {
    // 意图：root + 8 个无连接桩子代，锚距 = max(300, 200/(2·sin(π/8))≈261.2) = 300；
  ...
  it("无连接桩 20 子自动扩距成正多边形", () => {
    // 意图：root + 20 个无连接桩子代，理想锚距 = 200/(2·sin(π/20)) ≈ 639.24；
  ...
  it("带连接桩输入的确定性：多根多簇孤立节点图在乱序输入下输出一致", () => {
    // 意图：算法必须完全确定性，带连接桩的边参与计算时同样保证
    ...
    const buildInput = () => ({
      ...
      edges: [
        edgeWithHandles("root", "c1", "right", "left"),
        edgeWithHandles("root", "c2", "right", "left"),
        edgeWithHandles("root", "c3", "right", "left"),
        edgeWithHandles("c1", "g1", "right", "left"),
        edge("x1", "x2"),
      ],
    });
  ...
  it("防御：带连接桩的无效边被忽略", () => {
    // 意图：引用不存在节点的边（即使带有连接桩方向）应被忽略，
    // 两个节点都按孤立节点网格排列而非报错。
    const result = computeAutoLayout(
      [node("x"), node("y")],
      [edgeWithHandles("x", "ghost", "right", "left")],
      CONFIG,
    );
    expect(result.size).toBe(2);
    const x = result.get("x")!;
  ...
  it("菱形带捷径：多父节点保持各父的连接桩轴向间距", () => {
    ...
    const result = computeAutoLayout(
      ...
      [
        edgeWithHandles("a", "b1", "right", "left"),
        edgeWithHandles("a", "b2", "right", "left"),
        edgeWithHandles("b1", "c1", "right", "left"),
        edgeWithHandles("b2", "d1", "right", "left"),
        edgeWithHandles("c1", "d1", "right", "left"),
      ],
      CONFIG,
    );
    const a = result.get("a")!;
    ...
    // 连接桩轴向间距约束：d1 在 c1 正东 300（均值侵蚀被约束抵消）
  ...
  it("菱形带捷径（垂直方向）：bottom 连接桩保持垂直轴向间距", () => {
    ...
    const result = computeAutoLayout(
      [node("a"), node("b1"), node("b2"), node("c1"), node("d1")],
      [
        edgeWithHandles("a", "b1", "bottom", "top"),
        edgeWithHandles("a", "b2", "bottom", "top"),
        edgeWithHandles("b1", "c1", "bottom", "top"),
        edgeWithHandles("b2", "d1", "bottom", "top"),
        edgeWithHandles("c1", "d1", "bottom", "top"),
      ],
      CONFIG,
    );
    const a = result.get("a")!;
    const c1 = result.get("c1")!;
    const d1 = result.get("d1")!;
    // 连接桩轴向间距约束：d1 在 c1 正南 300（均值侵蚀被约束抵消）
  ...
  it("十字汇聚：多源多方位汇入中心时中心成为视觉根", () => {
    // 意图：四个节点从四个连接桩方位指向中心 C（N bottom→C top、
    ...
    const result = computeAutoLayout(
      ...
      [
        edgeWithHandles("N", "C", "bottom", "top"),
        edgeWithHandles("W", "C", "right", "left"),
        edgeWithHandles("E", "C", "left", "right"),
        edgeWithHandles("S", "C", "top", "bottom"),
      ],
      CONFIG,
    );
    const c = result.get("C")!;
  ...
  it("方向翻转对称：十字场景两种边方向产生相同布局", () => {
    ...
    const forward = computeAutoLayout(nodes, forwardEdges, CONFIG);
    const backward = computeAutoLayout(nodes, backwardEdges, CONFIG);
  ...
  it("汇聚带中间节点：反转保持连接桩语义链", () => {
    ...
    const result = computeAutoLayout(
      ...
      [
        edgeWithHandles("N", "M", "bottom", "top"),
        edgeWithHandles("M", "C", "bottom", "top"),
        edgeWithHandles("W", "C", "right", "left"),
        edgeWithHandles("E", "C", "left", "right"),
        edgeWithHandles("S", "C", "top", "bottom"),
      ],
  ...
  it("夹心汇聚：反向方位对反转成垂直排列", () => {
    ...
    const result = computeAutoLayout(
      ...
      [edgeWithHandles("a1", "b", "bottom"), edgeWithHandles("a2", "b", "top")],
      CONFIG,
    );
    const a1 = result.get("a1")!;
    const a2 = result.get("a2")!;
  ...
  it("方位一致的汇聚不反转：源在上汇在下", () => {
    ...
    const result = computeAutoLayout(
      [node("a1"), node("a2"), node("b")],
      [edgeWithHandles("a1", "b", "bottom", "top"), edgeWithHandles("a2", "b", "bottom", "top")],
      CONFIG,
    );
    const a1 = result.get("a1")!;
    const a2 = result.get("a2")!;
  ...
  it("发散型不反转：单源多汇保持源为根", () => {
    ...
    const result = computeAutoLayout(
      [node("root"), ...kids.map(node)],
      kids.map((id) => edgeWithHandles("root", id, "bottom", "top")),
      CONFIG,
    );
    const root = result.get("root")!;
  ...
  it("链式平局不反转：纯源=纯汇保持原方向", () => {
    // 意图：a→b→c 垂直连接桩链（纯源 1 = 纯汇 1，平局）不触发反转，
    // 布局沿边的原方向自上而下展开：a 上、b 中、c 下。
    // 这是防止过度反转的失败分支。
    const result = computeAutoLayout(
      [node("a"), node("b"), node("c")],
      [edgeWithHandles("a", "b", "bottom", "top"), edgeWithHandles("b", "c", "bottom", "top")],
      CONFIG,
    );
    const a = result.get("a")!;
    const b = result.get("b")!;
  ...
  it("分量级规范化：汇聚分量反转而相邻链分量不受影响", () => {
    ...
    const result = computeAutoLayout(
      ...
      [
        ...
        edgeWithHandles("E", "C", "left", "right"),
        edgeWithHandles("S", "C", "top", "bottom"),
        edgeWithHandles("p", "q", "bottom", "top"),
      ],
      CONFIG,
    );

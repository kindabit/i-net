 ...
 * 从不同连接桩方位汇聚到少数纯汇时（如十字：N bottom→C top、W right→C left、
 * E left→C right、S top→C bottom），连接桩语义要求汇点同时位于各源的多个
 ...
 *    "根行 + 均值"的现有正确行为。
 *
 * 【正确性】反转操作交换 source/target 与 sourceHandle/targetHandle；
 * 锚向解析（见 anchor.ts）本身对称——同一条物理连接无论哪个节点
 * 作为父节点，解析出的相对方位语义一致，故反转不改变连接桩语义。
 */

import { handleOf } from "./anchor";
import type { SanitizedGraph } from "./sanitize";
import type { AutoLayoutEdge, Handle } from "./types";
...
function shouldReverseComponent(
  ...
  const directions = new Set<Handle>();

<!--
  ...
  在画布左上角悬浮菜单右侧显示当前画布中已逻辑删除的节点列表。
  通过 props 接收节点列表，通过 emit 事件向父组件通知用户操作。
...
const hasCanvasRefNode = computed(() =>
  props.nodes.some((n) => n.canvas_ref_id !== null),
);
...
async function onPhysicalDelete(node: Node) {
  const confirmed = await confirmDialogRef.value?.open({
    title: t("database.canvas.physical-delete-title"),
    text: node.canvas_ref_id
      ? t("database.canvas.physical-delete-text-cascade", { title: node.title })
      : t("database.canvas.physical-delete-text", { title: node.title }),
    confirmColor: "error",
  });
  if (confirmed) emit("physicalDelete", node);
}

async function onEmpty() {
  const confirmed = await confirmDialogRef.value?.open({
    title: t("database.canvas.empty-recycle-bin-title"),
    text: hasCanvasRefNode.value
      ? t("database.canvas.empty-recycle-bin-text-cascade", {
          count: props.nodes.length,
        })
      : t("database.canvas.empty-recycle-bin-text", {
          count: props.nodes.length,
        }),
    confirmColor: "error",
  });
    ...
    >
      ...
      <div class="recycle-bin-list">
        ...
        >
          <div
            ...
            :class="{ 'recycle-bin-item--dragging': draggingId === node.id }"
            draggable="true"
          ...
          >
            ...
            <div class="recycle-bin-item-actions">
              <VBtn
                ...
                variant="text"
                density="comfortable"
                :title="t('database.canvas.restore-node')"
              ...
              <VBtn
                ...
                variant="text"
                density="comfortable"
                :title="t('database.canvas.physical-delete-node')"
                @click="onPhysicalDelete(node)"
              />
            </div>
          </div>
        </TransitionGroup>
        <Transition name="recycle-empty">
          <div v-if="nodes.length === 0" class="recycle-bin-empty">
            {{ t("database.canvas.recycle-bin-empty") }}
          </div>
        </Transition>
      </div>
      <div class="recycle-bin-footer">
        <VBtn
          ...
          @click="onEmpty"
        >
          {{ t("database.canvas.empty-recycle-bin") }}
        </VBtn>
      </div>
    </div>
  </Transition>
